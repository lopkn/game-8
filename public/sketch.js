...
function abActions() {
    ...
    if (currentlyDoing == 1 && TT.state != "start" && tempc != "no entity" && entities[tempc].team.teamname == TT.teamname) {
        if (selectedBuilding.name == "factory") {
            l.html(" act1: hire soldier<br> act2: hire architect<br> act3: buy tank<br> act4: hire medic<br> act5: hire sniper<br> click a tile near the factory after you've bought the unit to build it")
            ...
            l.html(" act1: move<br> act2: aim & shoot<br> act3: deploy soldier(more expensive than factory)")
        } else if (selectedBuilding.name == "sniper") {
        ...
        l.html(" you're trying to make a sniper. click a tile in a 2 block radius of your factory to build.")
...
function action5() {
    ...
    if (selectedBuilding.name == "factory" && TT.state == "building" && currentlyDoing == 1) {
        ...
        if (TT.money >= 300) {
            TT.money -= 300
            currentlyDoing = "sniper"
            console.log("making new:sniper")
        }
...
function clientSYNC(syncPacket) {
    ...
    for (let i = 0; i < tempEntities.length; i++) {
        ...
        if (en.name == "factory") {
            ...
            tempEntity = new trap()
        } else if (en.name == "sniper") {
            tempEntity = new sniper()
        }
...
function endTurn() {
    if (TT.state == "game" && currentlyDoing == 1) {
        ...
        turns += 1
        for (let i = 0; i < entities.length; i++) {
            if (entities[i].update != "noUpdate") {
                entities[i].UpdateMoves()
            }
        }
    }
}
...
function setup() {
    ...
    B3 = createButton('ACT 3');
    B3.position(200, newheight);
    B3.mousePressed(function() {
        allbuttonsemit(3)
    });
    B4 = createButton('ACT 4');
    B4.position(300, newheight);
    B4.mousePressed(function() {
        allbuttonsemit(4)
    });
    B5 = createButton('ACT 5');
    B5.position(400, newheight);
...
function update(fupdate, ex, ey) {
    ...
    if (fupdate == "sniper" && thisSelectedArea != null && dit < 2.1 && dit != 0 && cme == "no entity") {
        entities.push(new sniper(temp[0], temp[1], TT))
        currentlyDoing = 1
        TT.state = "game"
    }
    ...
    if (typeof(myTeamNum) != "undefined") {
        for (let i = entities.length - 1; i >= 0; i--) {
            ...
            if (entities[i].hp <= 0) {
                ...
                if (entities[i].team.teamname == teams[myTeamNum].teamname) {
                    entities[i].draw()
...
function draw() {
    if (started == true) {
        ...
        if (keywait <= 0) {
            if (keyIsDown(65)) {
                socket.emit('toserkd', 65);
                keywait = needwaittime
            }
            if (keyIsDown(87)) {
                socket.emit('toserkd', 87);
                keywait = needwaittime
            }
            if (keyIsDown(68)) {
                socket.emit('toserkd', 68);
                keywait = needwaittime
            }
            if (keyIsDown(83)) {
